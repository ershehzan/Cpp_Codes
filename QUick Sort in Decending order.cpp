...
// Elements greater than or equal to the pivot are placed to its left
// Elements smaller than the pivot are placed to its right
int partition(int arr[], int start, int end)
{
    ...
    for (int i = start; i <= end; i++)
    {
        ...
        {
            swap(arr[i], arr[pos]); // Place the larger element at the correct position
...
// This function sorts the array in descending order recursively
void QuickSort(int arr[], int start, int end)
{
    ...
    // Recursively sort the left half (elements greater than or equal to pivot)
    QuickSort(arr, start, pivot - 1);

    // Recursively sort the right half (elements smaller than pivot)
...
int main()
{
    ...
    // Call the QuickSort function to sort the array in descending order
    QuickSort(arr, 0, n - 1);
